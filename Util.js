...
function loadExternalCommands(commandList) {
    new Promise((resolve) => {
        fs.access(path.join(__dirname, "ExternalCommands"), fs.constants.F_OK, (err) => {
            if(!err) {
                resolve(true);
                return;
            }
        ...
        const commandDir = path.join(__dirname, 'ExternalCommands');
        const files = fs.readdirSync(commandDir).filter(file => file.endsWith(".js"));
        
        if (files.length === 0) {
            error("The external commands directory is empty!");
            return;
        }
